...
void loop(){
  ...
  if(digitalRead(A1) == LOW ){ //Kill switch is hit, don't do anything on this loop
    ...
    Serial.print("The load voltage is: ");
      ...
      //  Serial.println(prePCCVoltage);
       // delay(3000);
        ...
        if(!past11MS){
          ...
          if(turbineVoltage < 10){
            pitch.write(STARTUP_PITCH);
            currentPitch = STARTUP_PITCH;
            past11MS = false;
          }
          else{
            pitch.write(NEAR_STALL_PITCH);
            past11MS = true;
            currentPitch = NEAR_STALL_PITCH;
            safetyMaxVoltageCheck();
          }
...
void processDisconnectedState(boolean disconnected){
  if(disconnected){
    ...
    delay(6000); //DELAY SO IT DOES NOT PITCH BACK HERE
  }
  else{
...
boolean disconnectBetterCheck(){
  ...
  for(int i = 0; i < 10; i++){
    totalAveragePrePCCVoltages+=averageTurbineVoltage();
